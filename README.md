...
This was because the UI is really intuitive and basic. However, the project itself can be pretty technical for those unfamiliar to React, such as I once was.
...
## Features
- Anytime a change is made to the array (whether its updating its size, creating a new array, or sorting the array) the program will automatically update its visual to reflect your changes. In other words, the array
will always be in sync with the visual no matter what.
...
- React is popular, in-demand skill in the current industry. 
- It's single-page architecture was really useful for my project specifically because I wanted my program to update dynamically instead of re-directing users to different pages.
...
can re-render individually was seriously a game-changer. Now I _want_ to use React for all my future projects because it helped me create a project that I find more efficient and professional with added ease of coding. 

I was already familiar with HTML, CSS, JavaScript, and more uniquely, Java. I decided to learn TypeScript because it felt like a natural transition from Java whilst also allowing for a more structured, bug-free code.
The Typescript front-end handles everything to do with the array apart from visualizing it. One unique thing I learned is that you can assign multiple types to a variable using the "|" operator. This was a big takeaway 
for me because I thought Typescript was more rigid in how it handled typing. For example, I had to assign my useGraph two types: string and undefined. I initially had faced bugs because I was trying to initially define 
my JSON data as just a string when the backend could possibly return undefined. 
...
- how Components interaction with CSS 
